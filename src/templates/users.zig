...
const AddForm = struct {
    fn _render(name_err: ?[]const u8, email_err: ?[]const u8, writer: *std.Io.Writer) std.Io.Writer.Error!void {
        _ = &name_err;
        _ = &email_err;
        // users.zt:61
        ...
        if (name_err) |message| {
            // users.zt:65
            try writer.writeAll("<p class=\"field-error\">");
            try zt.writeEscaped(writer, message);
    ...
    fn _signature(_: ?[]const u8, _: ?[]const u8) void {}
...
pub const AddDialog = struct {
    fn _render(open: bool, name_err: ?[]const u8, email_err: ?[]const u8, writer: *std.Io.Writer) std.Io.Writer.Error!void {
        _ = &open;
        _ = &name_err;
        _ = &email_err;
        // users.zt:91
        if (open) {
            // users.zt:92
            try writer.writeAll("<dialog id=\"add-dialog\" open>");
            // users.zt:93
            try zt.renderComponent(AddForm, .{name_err, email_err}, writer);
            // users.zt:94
            try writer.writeAll("</dialog>");
        } else {
            // users.zt:96
            try writer.writeAll("<dialog id=\"add-dialog\">");
            // users.zt:97
            try zt.renderComponent(AddForm, .{name_err, email_err}, writer);
            // users.zt:98
            try writer.writeAll("</dialog>");
    ...
    fn _signature(_: bool, _: ?[]const u8, _: ?[]const u8) void {}
...
pub const Page = struct {
    fn _render(users: []const db.User, writer: *std.Io.Writer) std.Io.Writer.Error!void {
        ...
        // users.zt:163
        try writer.writeAll("<button class=\"primary\" data-on:click=\"document.getElementById('add-dialog').showModal()\">");
        try writer.writeAll("Add user");
        try writer.writeAll("</button>");
        // users.zt:164
        ...
        // users.zt:167
        try zt.renderComponent(AddDialog, .{false, null, null}, writer);
        // users.zt:168
